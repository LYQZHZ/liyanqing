...
class Ws_Param(object):
    # 初始化
    def __init__(self, APPID, APIKey, APISecret, AudioFile, Text,yz,nl):
        self.APPID = APPID
        self.APIKey = APIKey
        ...
        self.BusinessArgs = {"category": CATEGORY, "sub": SUB, "ent": yz, "cmd": "ssb", "auf": "audio/L16;rate=16000",
                             "aue": "raw", "text": self.Text, "ttp_skip": True, "aus": 1,"group":nl}
    ...
    try:
        ...
        if code != 0:
            ...
            if (status == 2):
                ...
                result_xml = ET.fromstring(xml.decode('gbk'))
                result_sentence=result_xml.find("read_sentence").find("rec_paper").find(shi)
                result2='准确率：'+result_sentence.get('accuracy_score')+"完整度："+result_sentence.get('integrity_score')+"流畅度："+result_sentence.get('fluency_score')
...
def juzifenxi(yinpin,text,yuzhong,nianling,shi1):
    global wsParam
    global shi
    shi = shi1
    wsParam = Ws_Param(APPID='27e70702', APISecret='MDJhZmNhM2YzMGI1MTQ4ZWRlMDAwODA5',
                       APIKey='3a9cd2838e85effa589be2c6d7eb4012',
                       AudioFile=yinpin,Text=text,yz=yuzhong,nl=nianling)
